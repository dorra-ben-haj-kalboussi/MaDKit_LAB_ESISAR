...
public class BeeLauncher extends madkit.kernel.Agent {
	...
	private static final int INITIAL_BEES_NB = 300;
	...
	protected void activate() {
		...
		BeeViewer beeViewer = new BeeViewer(beeScheduler, beesList);
	...
	protected void live() {
		while (isAlive()) {
			...
			if (randomMode) {
				killBees(false, 150);
				if (Math.random() < .2) {
					getLogger().info(() -> "Lanccccerrrrr HORNETTTTTSSS ATACKS");
					final Hornet newHornet = new Hornet();
					launchAgent(newHornet);
				} else if (Math.random() < .3) {
					if (Math.random() < .1) {
						if (queensList.size() > 1)
							if (queensList.size() > 2)
								killBees(true, (int) (Math.random() * 7) + 1);
							else
								killBees(true, 1);
					} else if (queensList.size() < 2)
						launchQueens((int) (Math.random() * 2) + 1);
				} else if (Math.random() < .9) {
					if (beesList.size() < 1000 && Runtime.getRuntime().freeMemory() > 100000) {
						launchBees((int) (Math.random() * 1000) + 500);
					}
				} else {
					killBees(false, (int) (Math.random() * 2) + 1);
				}
			}
		}
	}
